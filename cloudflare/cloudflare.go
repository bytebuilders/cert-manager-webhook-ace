...
func NewDNSProvider(dns01Nameservers []string, userAgent string) (*DNSProvider, error) {
	...
	return NewDNSProviderCredentials(baseURL, email, key, "", dns01Nameservers, userAgent)
}

// NewDNSProviderCredentials uses the supplied credentials to return a
// DNSProvider instance configured for cloudflare.
func NewDNSProviderCredentials(baseURL, email, key, token string, dns01Nameservers []string, userAgent string) (*DNSProvider, error) {
	if baseURL == "" {
...
func FindNearestZoneForFQDN(c DNSProviderType, fqdn string) (DNSZone, error) {
	...
	for i := 0; i < len(mappedFQDN)-1; i++ {
		from, to := len(mappedFQDN[i])+1, len(nextName)
		if from > to {
			continue
		}
