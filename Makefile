...
PRODUCT_NAME       := cert-manager-webhook-ace
ENFORCE_LICENSE    ?=

GO_PKG   := go.bytebuilders.dev
REPO     := $(notdir $(shell pwd))
BIN      := cert-manager-webhook-ace
COMPRESS ?= no
...
SRC_PKGS := cloudflare # directories which hold app source excluding tests (not vendored)
...
uninstall:
	@cd ../installer; \
	helm uninstall cert-manager-webhook-ace --namespace=$(KUBE_NAMESPACE) || true
...
add-license:
	...
	@docker run --rm 	                                 \
		...
		--env HTTPS_PROXY=$(HTTPS_PROXY)                 \
		$(BUILD_IMAGE)                                   \
		ltag -t "./hack/license" --excludes "vendor cloudflare" -v
...
check-license:
	...
	@docker run --rm 	                                 \
		...
		--env HTTPS_PROXY=$(HTTPS_PROXY)                 \
		$(BUILD_IMAGE)                                   \
		ltag -t "./hack/license" --excludes "vendor cloudflare" --check -v
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/cert-manager-webhook-ace run \
		--v=3 \
		--secure-port=8443 \
